...
async def _play_track(
    ...
    try:
        with logs.stage(
            ...
            pos = await player.play_or_queue(
                message.chat.id,
                track
            )
        ...
        asyncio.create_task(
            channel.log(
                *cui.now_playing(
                    ...
                    track.source,
                    chat_title,
                    _requester_name(message),
                    _requester_id(message)
                )
            )
        )
...
def _archive_info(
    ...
    return {
        ...
        "duration_text": _fmt_dur(
            int(rec.get("duration") or 0)
        ),
        "stream_url": "archive",
        "webpage_url": rec.get("url") or "",
        "thumbnail": None,
        "source": "archive",
        "archive_rec": rec,
    }
...
async def _from_database_bot(
    ...
    if not searchbot.enabled():
        LOGGER.info(
            "SEARCHBOT خاموش است (SEARCH_GROUP تنظیم نشده)"
        )
        return None
...
async def _search(
    ...
    if is_video:
        try:
            return await youtube.get_media(
                query,
                video=True
            )
        except Exception as e:  # noqa: BLE001
    ...
    if mode == platform_pref.SOUNDCLOUD:
        ...
        # ۱) خود دستور را مستقیم در ساوندکلاد بگرد
        sc = await soundcloud.search(query)

        if sc and sc.get("stream_url"):
            return sc

        # ۲) اسم دقیق را از یوتیوب بگیر
        exact = ""
...
async def _handle_play(
    ...
    # اگر روی یک فایل صوتی/ویدیویی تلگرام ریپلای شده
    if await _play_telegram_file(
        client,
        message
    ):
        return
    ...
    if not query:
        example = (
            "پخش فیلم هزارپا"
            if is_video
            else "پخش اهنگ شادمهر"
        )

        await message.reply_text(
            f"نام {'فیلم' if is_video else 'آهنگ'} "
            f"یا لینک را بده.\n"
            f"مثال: `{example}`"
        )
        return
...
# --- ریپلای روی فایل صوتی/ویدیویی ---
_TG_STREAM_THRESHOLD = (
    int(
        os.environ.get(
            "TG_STREAM_THRESHOLD_MB",
            "50"
        )
    )
    * 1024
...
async def _play_telegram_file(
    client: Client,
    message: Message
) -> bool:
    """اگر روی فایل صوتی/ویدیویی ریپلای شده، آن را پخش/به صف می‌کند."""
    ...
    if size >= _TG_STREAM_THRESHOLD:
        ...
        try:
            ...
            if (
                "GROUPCALL" in err.upper()
                or "no active" in err.lower()
            ):
                await _show(
                    status,
                    msg.no_voice_chat()
...
async def pause_cmd(
    ...
    await message.reply_text(
        "⏸ موقتاً متوقف شد. برای ادامه بنویس «ادامه»."
    )
